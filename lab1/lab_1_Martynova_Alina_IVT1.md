...
## Задача 3.1
### Постановка задачи
Для некоторого числового вектора `X`, введённого с клавиатуры, вычислить значения вектора `Y = X · X`, то есть выполнить поэлементное возведение в квадрат.

### Математическая модель
Пусть задан вектор `X = (x1, x2, ..., xn)`.  
Тогда элементы результирующего вектора определяются выражением:
...
## Задача 3.2
### Постановка задачи
Для числового массива `X`, введённого с клавиатуры поэлементно, изменить порядок элементов на обратный и распечатать результат.

### Математическая модель
Выполняется обмен элементов массива симметрично относительно середины:
...
Элементы транспонированной матрицы вычисляются по формуле:
...
`avg = (a[i][0] + a[i][1] + ... + a[i][m-1]) / m`

После этого первый элемент строки заменяется найденным значением:
...
| avg | double | среднее арифметическое |
...
## Задача 3.5
### Постановка задачи
Реализовать алгоритм сортировки вставками внутри функции `main`. Исходный массив вводится с клавиатуры, после чего выполняется сортировка по возрастанию.

### Математическая модель
Каждый элемент массива вставляется в уже отсортированную часть массива на нужную позицию.
...
|---|---|---|
| n | int | размер массива |
| i | int | счётчик внешнего цикла |
| j | int | счётчик внутреннего цикла |
| key | int | вставляемый элемент |
| a | int[] | массив |
